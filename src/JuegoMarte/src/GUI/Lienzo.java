...
public class Lienzo extends JPanel {
	...
	public void clear() {
		for (int y = 0; y < mGame.getTamanoY(); y++) {
			for (int x = 0; x < mGame.getTamanoX(); x++) {
				mGame.robot[x][y].setEstado(0);
				mGame.robot[x][y].setEstadoSiguiente(0);
			}
		}
	}

	public void updateMGame (int nFilas, int nColumnas) {
		mGame = new MatrizDeJuego(nFilas, nColumnas);
		this.nFilas = nFilas;
	...
	public void ponerObjeto(int x, int y) {
		...
		switch (mGame.robot[posX][posY].getEstado()) {
	        case 0:  mGame.robot[posX][posY].setEstado(1);
	                 break;
	        case 1:  mGame.robot[posX][posY].setEstado(2);
	                 break;
	        case 2:  mGame.robot[posX][posY].setEstado(3);
	        		 break;
	        case 3:  mGame.robot[posX][posY].setEstado(4);
   		 			 break;
	        case 4:  mGame.robot[posX][posY].setEstado(0);
		 			 break;
	        default: mGame.robot[posX][posY].setEstado(1);
	        		 break;
		}
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < mGame.getTamanoX(); i++) {
			...
			for (int j = 0; j < mGame.getTamanoY(); j++) {
				if (mGame.robot[i][j].getEstado() == 1) { // casilla pintada
